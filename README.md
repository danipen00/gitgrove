...
GitGrove's own source is [MIT](LICENSE).

It bundles open-source dependencies under their own permissive licenses — notably
the Apache-2.0 [`@pierre/trees`](https://trees.software) and
[`@pierre/diffs`](https://diffs.com). Their license texts and `NOTICE` files are
aggregated into `THIRD-PARTY-LICENSES.txt`, generated by `bun run licenses` from
the installed production dependency tree and shipped inside every distributable
(in the app's resources directory). The `dist` / `release` scripts regenerate it
